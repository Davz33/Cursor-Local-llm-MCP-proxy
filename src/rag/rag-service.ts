...
export class RAGService {
  ...
  constructor(storagePath: string = "./rag-storage") {
    ...
    try {
      ...
      console.error(
        `RAG Service [${this.instanceId}]: Initialized successfully`,
      );
      console.error(
        `RAG Service [${this.instanceId}]: Storage path: ${this.storagePath}`,
      );
      console.error(
        `RAG Service [${this.instanceId}]: Document index initialized as:`,
        this.documentIndex,
      );
    } catch (error) {
      console.error(
        `RAG Service [${this.instanceId}]: Failed to initialize:`,
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.error(
        `RAG Service [${this.instanceId}]: Initialization completed`,
      );
    } catch (error) {
      console.error(
        `RAG Service [${this.instanceId}]: Failed to initialize:`,
  ...
  private async loadStorage(): Promise<void> {
    try {
      if (await this.storageExists()) {
        console.error(
          `RAG Service [${this.instanceId}]: Loading existing storage from ${this.storagePath}`,
        );
        ...
        if (await this.fileExists(documentsFile)) {
          ...
          this.documents = savedDocuments.map(
            (doc: any) =>
              new Document({
                text: doc.text,
                id_: doc.id_,
          ...
          this.documentIndex = await VectorStoreIndex.fromDocuments(
            this.documents,
          );
          this.isPersistent = true;
          console.error(
            `RAG Service [${this.instanceId}]: Successfully loaded ${this.documents.length} documents from storage`,
          );
        } else {
          console.error(
            `RAG Service [${this.instanceId}]: No documents file found in storage`,
          );
        }
      } else {
        console.error(
          `RAG Service [${this.instanceId}]: No existing storage found at ${this.storagePath}`,
        );
      }
    } catch (error) {
      console.error(
        `RAG Service [${this.instanceId}]: Failed to load storage:`,
        (error as Error).message,
      );
      // Don't throw error - allow service to continue without persistence
    }
  ...
  async saveStorage(): Promise<void> {
    try {
      if (this.documentIndex && this.documents.length > 0) {
        console.error(
          `RAG Service [${this.instanceId}]: Saving storage to ${this.storagePath}`,
        );
        ...
        this.isPersistent = true;
        console.error(
          `RAG Service [${this.instanceId}]: Successfully saved ${this.documents.length} documents to persistent storage`,
        );
      } else {
      ...
      console.error(
        `RAG Service [${this.instanceId}]: Failed to save storage:`,
        (error as Error).message,
      );
      throw new Error(
  ...
  async clearStorage(): Promise<void> {
    try {
      if (await this.storageExists()) {
        console.error(
          `RAG Service [${this.instanceId}]: Clearing storage at ${this.storagePath}`,
        );
        await fs.rm(this.storagePath, { recursive: true, force: true });
        this.isPersistent = false;
        console.error(
          `RAG Service [${this.instanceId}]: Storage cleared successfully`,
        );
      }
    } catch (error) {
      console.error(
        `RAG Service [${this.instanceId}]: Failed to clear storage:`,
        (error as Error).message,
      );
      throw new Error(
  ...
  async indexDocument(filePath: string): Promise<string> {
    try {
      ...
      if (!this.documentIndex) {
        console.error(
          "RAG Service: Creating new VectorStoreIndex from document",
        );
  ...
  async indexText(text: string): Promise<string> {
    try {
      console.error(
        `RAG Service [${this.instanceId}]: Starting indexText with text length:`,
        text.length,
      );
      ...
      if (!this.documentIndex) {
        console.error(
          "RAG Service: Creating new VectorStoreIndex from text document",
        );
        this.documentIndex = await VectorStoreIndex.fromDocuments([
          textDocument,
        ]);
        console.error(
          "RAG Service: VectorStoreIndex created successfully from text",
        );
      } else {
      ...
      console.error(
        "RAG Service: Error in indexText:",
        (error as Error).message,
      );
  ...
  async queryDocuments(query: string): Promise<RAGQueryResult> {
    try {
      ...
      console.error(
        `RAG Service [${this.instanceId}]: Starting queryDocuments with query:`,
        query,
      );

      if (!this.documentIndex) {
        console.error(
          `RAG Service [${this.instanceId}]: No document index found`,
        );
      ...
      console.error(
        `RAG Service [${this.instanceId}]: Document index exists, creating query engine`,
      );
      const queryEngine = this.documentIndex.asQueryEngine();
      console.error(
        `RAG Service [${this.instanceId}]: Query engine created successfully`,
      );
      ...
      console.error(
        `RAG Service [${this.instanceId}]: Query executed, response type:`,
        typeof response,
      );
      console.error(
        `RAG Service [${this.instanceId}]: Response keys:`,
      ...
      try {
        ...
        if (
          ...
          sourceInfo = response.sourceNodes
            .map((node) => {
        ...
        console.error(
          `RAG Service [${this.instanceId}]: Error parsing response:`,
      ...
      console.error(
        `RAG Service [${this.instanceId}]: Successfully processed response`,
      );
      ...
      console.error(
        `RAG Service [${this.instanceId}]: Error in queryDocuments:`,
        (error as Error).message,
      );
      console.error(
        `RAG Service [${this.instanceId}]: Error stack:`,
  ...
  hasIndexedDocuments(): boolean {
    console.error(
      "RAG Service: Checking if documents are indexed:",
